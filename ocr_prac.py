...
ocr = PaddleOCR(
    ...
    use_angle_cls=True,
    rec_algorithm="CRNN",
    rec_model_dir="PaddleOCR/PaddleOCR/output/rec_ppocr_v3_korean/best_model",
)
...
def draw_annotations(image, annotations):
    ...
    if license_plate_bbox and detected_numbers:
        ...
        if results:
            ...
            # OCR 결과에서 한글 문자 추출 및 수정
            korean_char = extract_korean_char(ocr_text)

            # YOLO로 탐지된 숫자와 OCR로 인식된 한글 문자 결합
            front_numbers = (
...
if __name__ == "__main__":
    input_path = "realtest/carnum6.jpg"
    main(input_path)
